...
const schema = z.object({
  statement_of_purpose: z
    .string()
    .min(1500, { message: "Minimum 1500 characters are required" }),
});
const PersonalStatementForm = () => {
  ...
  const config = useMemo(() => ({
    readonly: false,
    placeholder: "Maximum 1 Page, Single-Spaced (~500-600 words)",
  }));
  ...
  useEffect(() => {
    if (application) {
      setContent(application.statement_of_purpose);
    ...
    <>
      ...
      <div className="w-full  lg:rounded-[60px] lg:p-[60px] sm:p-[20px] sm:mt-0 md:mt-[20px] flex-col bg-white h-fit ">
        ...
        <p className=" font-thin mb-4">
          Your Statement of Purpose helps us get to know you beyond your
          transcripts and resume. It's an opportunity to share what draws you to
          this program, what experiences have shaped your path, and how you hope
          to grow through your time at the Becoming Institute. Please focus on
          what feels most relevant to your journey, your readiness, and your
          intentions for the work ahead.
        </p>
  ...
  render={({ field }) => (
    <JoditEditor
      ref={editor}
      value={field.value || ""} // Use field value to keep it in sync
      config={config}
      onBlur={(newContent) => {
        field.onChange(newContent); // Very important
        setContent(newContent); // Optional, for local state if needed
      }}
      onChange={() => {}} // Can be empty or removed
    />
  )}
/>
